...
export function Dashboard() {
  ...
  return (
    <section id="dashboard" className="bg-bg text-content-primary border-t border-surface-border">
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-24 md:py-32">
        <div className="max-w-2xl mb-14">
          ...
          <p className="mt-3 text-content-secondary text-lg">{t("dash.sub")}</p>
        </div>
...
function Panel({
  ...
  return (
    <div className={`rounded-xl bg-panel border border-surface-border ${className}`}>
      <div className="px-5 py-3 border-b border-surface-border flex items-baseline justify-between">
        <div className="font-mono-tabular text-xs uppercase tracking-wider text-content-primary">{title}</div>
        {subtitle && <div className="font-mono-tabular text-xs text-content-secondary">{subtitle}</div>}
      </div>
...
function ShareOfVoice({ items }: { items: Brand[] }) {
  ...
  return (
    <div ref={ref}>
      <div className="flex h-14 w-full rounded-md overflow-hidden border border-surface-border">
        {items.map((b, i) => (
          <div
            key={b.name}
          ...
          >
            {b.share >= 12 && (
              <span className={`font-mono-tabular text-xs ${b.you ? "text-white" : "text-content-primary/70"}`}>
                {b.share}%
              </span>
            )}
            {hover === i && (
              <div className="absolute -top-11 left-1/2 -translate-x-1/2 whitespace-nowrap bg-surface text-content-primary text-xs px-2.5 py-1.5 rounded-md font-mono-tabular shadow-lg z-10 border border-surface-border">
                {b.name} пїЅ {b.share}%
              </div>
            )}
          </div>
      ...
      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs">
        {items.map((b) => (
          <div key={b.name} className="flex items-center gap-2 text-content-secondary">
            <span className="w-2.5 h-2.5 rounded-sm" style={{ background: b.color }} />
            <span className={b.you ? "text-content-primary font-medium" : ""}>{b.name}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="overflow-x-auto -mx-5 px-5">
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left font-mono-tabular text-[10px] uppercase tracking-wider text-content-secondary border-b border-surface-border">
        ...
        <tbody>
          {runs.map((r, i) => (
            <tr key={i} className="border-b border-surface-border last:border-0 hover:bg-surface-hover">
              ...
              <td className="py-3 pr-4 font-mono-tabular text-xs text-content-secondary"><div className="flex items-center gap-1.5"><AILogo model={r.model} className="w-3.5 h-3.5 object-contain opacity-70" />{r.model}</div></td>
              <td className="py-3 pr-4">
                {r.mentioned ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 text-content-secondary font-mono-tabular text-xs">
                    <span className="w-1.5 h-1.5 rounded-full bg-content-muted" />
                    {t("dash.no")}
                  </span>
                )}
              </td>
              <td className="py-3 pr-4 font-mono-tabular">{r.position ? `#${r.position}` : "2"}</td>
              <td className="py-3 pr-4 font-mono-tabular">
                {r.mentioned ? (
                  <span
                    className={
                      r.sentiment > 0.5 ? "text-positive" : r.sentiment > 0.2 ? "text-content-primary" : "text-negative"
                    }
                  >
                    {r.sentiment > 0 ? "+" : ""}
              ...
              <td className="py-3 font-mono-tabular text-content-secondary">{r.sources || "2"}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div>
      <div className="flex items-baseline justify-between mb-3">
        <div>
          ...
          <div className="text-xs text-content-secondary">{t("dash.mentionsToday")}</div>
        </div>
      ...
      <div className="flex justify-between text-[10px] font-mono-tabular text-content-secondary mt-1">
      ...
      {items.map((s) => (
        ...
        >
          <div>
            ...
            <div className="text-xs text-content-secondary mt-0.5">{s.note}</div>
          </div>
          <div className="text-right">
            <div className="font-mono-tabular text-lg">{s.citations}</div>
            <div className="text-[10px] text-content-secondary uppercase tracking-wider">{t("dash.citations")}</div>
          </div>
        </div>
      ))}
    </div>
